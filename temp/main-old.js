(function () {
    ...
    if (window.document) {
        ...
        var arrayBufferHandler = (item, callback, isCrossOrigin, img) => {
            ...
            var data = window.resMap[item.url];
            console.log('loadImg:'+ item.url);
            var img = new Image();
            function loadCallback() {
                img.removeEventListener('load', loadCallback);
                img.removeEventListener('error', errorCallback);

                console.log('loadImg1:'+ item.url);

                callback(null, img);
            }
            function errorCallback() {
                img.removeEventListener('load', loadCallback);
                img.removeEventListener('error', loadCallback);
                ...
                console.log('loadImg0:'+ item.url);
                callback(new Error('Load image (' + item.url + ') failed'));
            }
            if (data != null) {
        ...
        var jsonBufferHandler = (item, callback) => {
            ...
            console.log('loadjson:'+ item.url);
            callback(null, str);
        };

        // web加载音频
        var audioBufferHandler = (item, callback) => {
            console.log('loadaudio:'+ item.url);
